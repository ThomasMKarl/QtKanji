...
struct Flashcard
{
  ...
  {
    if (dataSign != rhs.dataSign)
      return false;
    if (dataImiVector != rhs.dataImiVector)
      return false;
    if (dataOnVector != rhs.dataOnVector)
      return false;
    if (dataKunVector != rhs.dataKunVector)
      return false;
    return true;
  }
};
...
template <typename T> void getSortedSubarray(std::vector<T> &array, const size_t lowerLimit, const size_t upperLimit)
{
...
template <typename T> bool contains(const std::vector<T> &vector, const T &value)
{
  if (vector.empty())
    return false;
...
template <typename T> bool oneInVector(const std::vector<T> &values, const std::vector<T> &vector)
{
  if (values.empty() || vector.empty())
    return false;
  for (size_t i = 0; i < values.size(); ++i)
    if (contains(vector, values[i]))
      return true;
  return false;
}

template <typename T> bool allInVector(const std::vector<T> &values, const std::vector<T> &vector)
{
  if (values.empty() || vector.empty())
    return false;
  for (size_t i = 0; i < values.size(); ++i)
...
std::set<T> overlap(const std::vector<T> &copy0, const std::vector<T> &copy1, const std::vector<T> &copy2)
{
  ...
  if (copy0.empty() && copy1.empty() && copy2.empty())
    return result;

  if (copy0.empty() && copy1.empty())
  {
    for (const auto &index : copy2)
      result.insert(index);
    return result;
  }
