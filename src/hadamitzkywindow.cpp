...
QtKanji::HadamitzkyWindow::HadamitzkyWindow(QWidget *parent): QWidget(parent)
{
  ...
  computeRadicalKanjiMap_asyncHandle = std::async(std::launch::async, &QtKanji::HadamitzkyWindow::computeRadicalKanjiMap, this);
}
...
void QtKanji::HadamitzkyWindow::graphemeButtonClicked(unsigned int graphemeIndex)
{
  ...
  if(selectedRadicals[0] != 0) copy0 = maps.radicalKanjiMap[selectedRadicals[0]-1];  
  if(selectedRadicals[1] != 0) copy1 = maps.radicalKanjiMap[selectedRadicals[1]-1];
  if(selectedRadicals[2] != 0) copy2 = maps.radicalKanjiMap[selectedRadicals[2]-1];
  ...
  for(const auto index : kanjis)
  {
    ...
    connect(&kanjiButtons[buttonNumber],
	          &QPushButton::clicked,
	          this,
            [=](){ kanjiClicked(index); });
...
QtKanji::Error QtKanji::HadamitzkyWindow::printSigns(unsigned int lowerLimit, unsigned int upperLimit) const
{
  ...
  if(!htmlFile || !texFile) return Error::FILE_ERROR;
  ...
  for(const auto &radicalStrokeNumberMap : maps.radicalStrokeNumberMaps)
  {
    ...
    {  
      ...
      {
        DataHandler data{};
        if(data.computeKanjiData(index) != QtKanji::Error::SUCCESS) continue;
